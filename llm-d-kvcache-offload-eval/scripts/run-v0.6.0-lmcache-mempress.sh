...
# LMCache benchmarks for llm-d v0.6.0 at reduced gpu_memory_utilization (memory-pressure).
# Same gmu values as v0.5.1-lmcache-mempress for direct cross-version comparison.
# Uses lmcache/vllm-openai:v0.4.2 (released 2026-03-17, targets vLLM 0.17.x).
...
for replicas in ${REPLICAS}; do
    ...
    for model in ${MODELS}; do
        ...
        # Per-model gpu_memory_utilization (same as v0.5.1-lmcache-mempress)
        case "${MODEL_NAME}" in
        ...
        # LMCache CPU cache size per model (same as v0.5.1 for comparability)
        case "${MODEL_NAME}" in
        ...
        for run in ${RUNS}; do
            ...
            case "${run}" in
                "lmcache-local")
                    export CONTAINER_IMAGE="docker.io/lmcache/vllm-openai:v0.4.2"
...
echo ""
echo "=========================================="
echo "v0.6.0 lmcache mempress benchmarks complete!"

...
# v0.6.0 benchmarks with per-model gpu_memory_utilization tuned for memory pressure.
# Same gmu values as v0.5.1-mempress for direct cross-version comparison.
...
export SOFTWARE="${SOFTWARE:-upstream-llm-d-0.6.0-mempress}"
export TURNS=5
...
for replicas in ${REPLICAS}; do
    ...
    for model in ${MODELS}; do
        ...
        # Per-model gpu_memory_utilization (same as v0.5.1-mempress)
        case "${MODEL_NAME}" in
...
echo ""
echo "=========================================="
echo "v0.6.0 memory-pressure benchmarks complete!"

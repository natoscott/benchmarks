...
# v0.7.0 benchmarks with per-model gpu_memory_utilization tuned for memory pressure.
# Same gmu values as v0.5.1-mempress and v0.6.0-mempress for direct cross-version comparison.
...
# llmd_fs_connector wheel — v0.19+cu130 for vLLM 0.19.1 / CUDA 13.0
...
for replicas in ${REPLICAS}; do
    ...
    for model in ${MODELS}; do
        ...
        # Per-model gpu_memory_utilization (same as v0.5.1-mempress and v0.6.0-mempress)
        case "${MODEL_NAME}" in
        ...
        for run in ${RUNS}; do
            ...
            case "${run}" in
                ...
                "cpu+fs-offload-20k")
                    ...
                    export EPP_BACKEND_CONFIG="in-memory"
                    export USE_LMCACHE_IMAGE=""
                    export VLLM_PRE_CMD="rm -rf ${FS_PACKAGES_DIR} && pip3.12 install --quiet --no-cache-dir --target ${FS_PACKAGES_DIR} ${FS_WHEEL_PATH} && mkdir -p /kvcache/kv-cache && python3.12 /data/patch_multi_connector.py"
                    ;;
                *)
...
echo ""
echo "=========================================="
echo "v0.7.0 memory-pressure benchmarks complete!"

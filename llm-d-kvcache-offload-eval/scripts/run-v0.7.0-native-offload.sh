...
# Benchmarks for llm-d v0.7.0 (vLLM 0.19.1) — no-offload and native CPU offload.
# Mirrors run-v0.6.0-native-offload.sh for apples-to-apples version comparison.
...
export SOFTWARE="${SOFTWARE:-upstream-llm-d-0.7.0}"
export TURNS=5
...
for replicas in ${REPLICAS}; do
    ...
    for model in ${MODELS}; do
        ...
        # cpu_bytes_to_use for native-offload-20k (same values as v0.5.1/v0.6.0 — hardware unchanged)
        case "${MODEL_NAME}" in
...
echo ""
echo "=========================================="
echo "v0.7.0 native offload benchmarks complete!"

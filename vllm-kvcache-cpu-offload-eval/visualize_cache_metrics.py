...
# Load data
df = pd.read_csv('cache_analysis_results.csv')
...
# Plot 3: Average cache query rate comparison
...
ax3.set_ylabel('Avg Cache Queries/sec', fontsize=11)
ax3.set_title('Average Cache Query Rate', fontsize=12, fontweight='bold')
ax3.set_xticks(x + width / 2)
ax3.set_xticklabels([m.split('-')[1] for m in models])
ax3.legend()
ax3.grid(True, alpha=0.3, axis='y')

# Plot 4: Cache hit rate vs Concurrency - Qwen3-0.6B
ax4 = fig.add_subplot(gs[1, 0])
for config in ['default', 'offload']:
    data = df[(df['model'] == 'Qwen3-0.6B') & (df['config_label'] == config)]
    ax4.plot(data['concurrency'], data['connector_hit_rate_mean'], marker='o', label=config,
            color=colors[config], linewidth=2, markersize=8)
ax4.set_xlabel('Concurrency', fontsize=11)
ax4.set_ylabel('Cache Hits/sec', fontsize=11)
ax4.set_title('Qwen3-0.6B: Cache Hit Rate', fontsize=12, fontweight='bold')
ax4.legend()
ax4.grid(True, alpha=0.3)
ax4.set_xscale('log')

# Plot 5: Cache hit rate vs Concurrency - Qwen3-8B
...
ax5.set_title('Qwen3-8B: Cache Hit Rate', fontsize=12, fontweight='bold')
ax5.legend()
ax5.grid(True, alpha=0.3)
ax5.set_xscale('log')

# Plot 6: Average cache hit rate comparison
...
ax6.set_ylabel('Avg Cache Hits/sec', fontsize=11)
ax6.set_title('Average Cache Hit Rate', fontsize=12, fontweight='bold')
ax6.set_xticks(x + width / 2)
ax6.set_xticklabels([m.split('-')[1] for m in models])
ax6.legend()
ax6.grid(True, alpha=0.3, axis='y')

# Plot 7: Cache hit ratio vs Concurrency - Qwen3-0.6B
ax7 = fig.add_subplot(gs[2, 0])
for config in ['default', 'offload']:
    data = df[(df['model'] == 'Qwen3-0.6B') & (df['config_label'] == config)]
    ax7.plot(data['concurrency'], data['connector_hit_ratio'], marker='o', label=config,
            color=colors[config], linewidth=2, markersize=8)
ax7.set_xlabel('Concurrency', fontsize=11)
ax7.set_ylabel('Cache Hit Ratio (%)', fontsize=11)
ax7.set_title('Qwen3-0.6B: Cache Hit Ratio', fontsize=12, fontweight='bold')
ax7.legend()
ax7.grid(True, alpha=0.3)
ax7.set_xscale('log')
ax7.set_ylim(0, 100)

# Plot 8: Cache hit ratio vs Concurrency - Qwen3-8B
ax8 = fig.add_subplot(gs[2, 1])
for config in ['default', 'offload']:
    data = df[(df['model'] == 'Qwen3-8B') & (df['config_label'] == config)]
    ax8.plot(data['concurrency'], data['connector_hit_ratio'], marker='o', label=config,
            color=colors[config], linewidth=2, markersize=8)
ax8.set_xlabel('Concurrency', fontsize=11)
ax8.set_ylabel('Cache Hit Ratio (%)', fontsize=11)
ax8.set_title('Qwen3-8B: Cache Hit Ratio', fontsize=12, fontweight='bold')
ax8.legend()
ax8.grid(True, alpha=0.3)
ax8.set_xscale('log')
ax8.set_ylim(0, 100)

# Plot 9: Average cache hit ratio comparison
...
for i, config in enumerate(['default', 'offload']):
    ...
    for model in models:
        ...
        values.append(avg)
    bars = ax9.bar(x + i*width, values, width, label=config, color=colors[config], alpha=0.8)
...
ax9.set_ylabel('Avg Cache Hit Ratio (%)', fontsize=11)
ax9.set_title('Average Cache Hit Ratio', fontsize=12, fontweight='bold')
ax9.set_xticks(x + width / 2)
ax9.set_xticklabels([m.split('-')[1] for m in models])
ax9.legend()
...
x = np.arange(2)
width = 0.35
bars1 = ax.bar(x - width/2, hit_ratio_06b, width, label='0.6B', color=palette_colors[2], alpha=0.8)
bars2 = ax.bar(x + width/2, hit_ratio_8b, width, label='8B', color=palette_colors[3], alpha=0.8)
